# SparkLogProcessorWithLoggingnE - E is for elaborate
import re
import datetime
...
spark = SparkSession.builder.appName('Spark Log Processing Elaborate') \
    .getOrCreate()
...
# use the custom logging module created
try:
    sc.addPyFile(argv[1])
except:
    pass
...
def parseApacheLogLine(logline):
    ...
    if size_field == '-':
        logger.info(f'Replacing the - in record {logline} with a zero')
        size = int(0)
    else:
...
# time.sleep(3600)
# submit the application to yarn provide the spark_simple_logging.y file location as hdfs location
# spark-submit --master yarn SparkLogProcessorWithLoggingE.py hdfs://master:8020/user/vagrant/spark_lggging.py apachelogs50k.gz
# yarn application -list --appStates all
# yarn logs -applicationId application_1642144979924_0007 -log_files pyspark.log