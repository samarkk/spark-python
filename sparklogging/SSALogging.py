...
def parseApacheLogLine(logline):
    ...
    if size_field == '-':
        logging.warning(f'Replacing the - in record {logline} with a zero')
        size = int(0)
    else:
...
if argv[2] == 'dbg':
    pydevd.settrace(host=argv[3],port=5678,stdout_to_server=True,stderr_to_server=True)
...
print(avg_daily_requests_per_host)
# time.sleep(3600)
# spark-submit --driver-java-options "-Dlog4j.configuration=file:///vagrant/log4j-custom.properties" --conf spark.executor.extraJavaOptions=-"-Dlog4j.configuration=file:///vagrant/log4j-custom.properties" /vagrant/SparkLogProcessorWithLoggingSA.py --master  spark://master:7077  spark_simple_logging.py apachelogs50k.gz

# notes
# so far had to install pydevd using pip3 on the nodes for pydevd to work
# for pydevd to be installed on anaocnda use channel props
# way using pydevd above we are debugging only the driver
# and maybe not even that
# will have to wrap the function as per pyspark remote debugging to make
# executor debugging work